...
function external_module() {
    if [ $1 = "blogging.product.standard" ]; then
        cp external/StandardBlog.jar $product/
        cp external/blog.page.core.jar $product/
        cp external/blog.page.comment.jar $product/
        cp external/blog.page.share.jar $product/
    fi
}
